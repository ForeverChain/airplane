...
exports.getOneWayFlights = async (req, res) => {
  try {
    ...
    // Assuming Flights model has a method to fetch flights based on the provided parameters
    ...
    // You can perform additional filtering or sorting here if required
...
exports.removeFlight = async (req, res) => {
  try {
    const result = await Flights.remove({
    ...
    await res
      .status(400)
      .json({ message: "could not delete flight", error: e.message });
  }
};
...
exports.addFlights = async (req, res) => {
  try {
    ...
    res.json({ success: true, flights: savedFlight });
  } catch (error) {
